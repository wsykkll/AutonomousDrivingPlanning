...
struct ROAD_CONNECTION_OUT
{
	CONNECT_TYPE enConnectType;
	int iOutRoadID;
};
...
{
	int iInRoadID;
	std::vector<ROAD_CONNECTION_OUT> vec_RoadOuts;
};
...
struct METIntersection
{
	...
	std::vector<ROAD_CONNECTION> vec_RoadConnections;
...
struct METSection
{
	...
	METBox stSectionBox;
	ROADTYPE enRoadType;
	METSection* pLastSection;
	METSection* pNextSection;
	bool bConnected;//该Section是否与路口相连
	int nConnectedID;
	METIntersection* pConnectedInterSection;
	std::vector<MET_POI> vec_pois;
